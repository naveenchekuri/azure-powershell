...
namespace Microsoft.Azure.Commands.Network
{
    ...
    public partial class GetAzureRmVirtualRouter : NetworkBaseCmdlet
    {
        [Parameter(
        ...
        public override void Execute()
        {
            ...
            if (ShouldGetByName(ResourceGroupName, RouterName))
            {
                ...
                foreach (var peering in vVirtualRouter.Peerings)
                {
                    var resourceInfo = new ResourceIdentifier(peering.Id);
                    var vVirtualRouterPeering = this.NetworkClient.NetworkManagementClient.VirtualRouterPeerings.Get(ResourceGroupName, RouterName, resourceInfo.ResourceName);
                    vVirtualRouterModel.Peerings.Add(NetworkResourceManagerProfile.Mapper.Map<CNM.PSVirtualRouterPeer>(vVirtualRouterPeering));
                }
            ...
            else
            {
                ...
                WriteObject(TopLevelWildcardFilter(ResourceGroupName, RouterName, psVirtualRouterList), true);
            }
        }
    }
}
